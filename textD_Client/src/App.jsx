    ...
    Sube una imagen para extraer el texto y descubre lo que puedes hacer con él.
      ...
      <button
        ...
        className={`px-4 py-2 text-white rounded-md transition-colors duration-300 ${
          isTranslatorMode ? 'bg-accent hover:bg-accentHover ' : 'bg-gray-600 hover:bg-gray-700 mb-4'
        }`}
      >
      ...
      {isTranslatorMode ? <TranslatorPanel  sourceText='Image uploader' /> : <CopyableText text="Image uploader" />}
    </div>
        </div>
        <Footer />
      </div>
